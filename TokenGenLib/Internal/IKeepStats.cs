...
namespace TokenGenLib.Internal
{
  ...
  {
    private readonly IList<ITokenRepository> _tokenRepos; // GTan: this will fail if we inject only one ITokenRepository, so think of using IActivator to get services instead of constructor injection.

    public StatsKeeper(IList<ITokenRepository> tokenRepos)
    {
      _tokenRepos = tokenRepos;
      //_tokenRepo.TokenIssued += UpdateStats; // GTan: Need to fix this..
    }

    private void UpdateStats(object sender, TokenIssuedEventArgs e)
    {
      // GTan: Update public statistics.
    }

    public void UpdateStats(int tokensCount)
    {
      // GTan: Not Used, may need to remove the method.
    }
    ...
    protected virtual void Dispose(bool disposing)
    {
      if (!disposedValue)
      {
        if (disposing)
        {
          //_tokenRepo.TokenIssued -= UpdateStats; // GTan: Fix this too.
        }
